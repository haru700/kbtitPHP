 ...
 *
 * This class provides methods for accessing the response of a MA
 * request.
 *
 * @category  Services
 * @package   Services_Yahoo_JP
 ...
 * @link      http://phpize.net
 */
class Services_Yahoo_JP_MA_Response implements Iterator
{
    /**
     * validate flag
     *
    ...
    public function __construct(HTTP_Request $request)
    {
        ...
        if ($this->_isError() == true) {
            $exception = new Services_Yahoo_Exception("MA query failed");
    ...
    {
        return count((array)$this->xml->{$this->_getResultType()}->word_list->word);
    }
    ...
    {
        return 
            (array)$this->xml->{$this->_getResultType()}->word_list->word[$this->_iteratorCounter];
    }
    ...
    public function next()
    {
        ...
        if (!isset($this->xml->{$this->_getResultType()}->word_list->word[$this->_iteratorCounter])) {
     ...
     */
    private function _getResultType()
    {
        if (isset($this->_request->_url->querystring['results'])) {
            return $this->_request->_url->querystring['results'] . '_result';
        } else {
            return 'ma_result';
        }
    }
